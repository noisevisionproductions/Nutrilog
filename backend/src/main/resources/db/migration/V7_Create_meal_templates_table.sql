...
INSERT INTO meal_templates (external_id, name, instructions, meal_type, category, calories, protein, fat, carbs, usage_count) VALUES
('owsianka_z_owocami_001', 'Owsianka z owocami', 'Ugotuj płatki owsiane na mleku, dodaj pokrojone owoce i miód.', 'BREAKFAST', 'healthy', true, 320, 12, 8, 52, 15),
('salatka_cezar_002', 'Sałatka Cezar', 'Wymieszaj sałatę z kurczakiem, grzankami i sosem cezar.', 'LUNCH', 'salads', true, 420, 28, 24, 18, 8),
('kanapka_z_awokado_003', 'Kanapka z awokado', 'Rozgnieć awokado na grzance pełnoziarnistej, dodaj sól i pieprz.', 'BREAKFAST', 'sandwiches', true, 280, 8, 16, 26, 12);

-- Dodanie przykładowych składników
INSERT INTO meal_template_ingredients (meal_template_id, name, quantity, unit, original_text, display_order) VALUES
((SELECT id FROM meal_templates WHERE external_id = 'owsianka_z_owocami_001'), 'płatki owsiane', 50, 'g', '50g płatków owsianych', 1),
((SELECT id FROM meal_templates WHERE external_id = 'owsianka_z_owocami_001'), 'mleko', 200, 'ml', '200ml mleka', 2),
((SELECT id FROM meal_templates WHERE external_id = 'owsianka_z_owocami_001'), 'banan', 1, 'szt', '1 banan', 3),
((SELECT id FROM meal_templates WHERE external_id = 'owsianka_z_owocami_001'), 'miód', 1, 'łyżka', '1 łyżka miodu', 4);
