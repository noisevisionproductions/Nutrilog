import Container from "../../ui/landing/Container";
import Logo from "../../ui/landing/Logo";
import {footerLinks} from './footerData';

const Footer = () => {
    const currentYear = new Date().getFullYear();

    return (
        <footer className="bg-surface-dark">

            {/* Main footer content */}
            <div className="bg-surface-dark py-8 border-t border-border/10">
                <Container>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
                        {/* Brand section */}
                        <div>
                            <Logo/>
                            <p className="mt-3 text-xs text-text-secondary-dark">
                                Kompleksowy system do zarządzania praktyką dietetyczną i komunikacji z klientami.
                            </p>
                            <div className="mt-4 flex space-x-4">
                                {/*<a href="#" className="text-text-secondary-dark hover:text-primary">
                                    <span className="sr-only">Facebook</span>
                                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path fillRule="evenodd"
                                              d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                                              clipRule="evenodd"/>
                                    </svg>
                                </a>*/}
                                <a href="https://discord.gg/WSqaQbcc"
                                   className="text-text-secondary-dark hover:text-primary" target="_blank"
                                   rel="noopener noreferrer">
                                    <span className="sr-only">Discord</span>
                                    <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path
                                            d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/>
                                    </svg>
                                </a>
                            </div>
                        </div>

                        {/* Links sections */}
                        {footerLinks.map((section) => (
                            <div key={section.title}>
                                <h3 className="font-medium text-text-primary-dark text-sm mb-3">
                                    {section.title}
                                </h3>
                                <ul className="space-y-2">
                                    {section.links.map((link) => (
                                        <li key={link.text}>
                                            <a
                                                href={link.href}
                                                className="text-xs text-text-secondary-dark hover:text-primary transition-colors duration-200"
                                            >
                                                {link.text}
                                            </a>
                                        </li>
                                    ))}
                                </ul>
                            </div>
                        ))}
                    </div>

                    {/* Bottom bar */}
                    <div className="border-t border-border/10 mt-8 pt-6">
                        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                            <p className="text-xs text-text-secondary-dark">
                                © {currentYear} NutriLog. Wszystkie prawa zastrzeżone.
                            </p>
                            {/*  <div className="flex gap-6">
                                <Link
                                    to="/privacy-policy"
                                    className="text-xs text-text-secondary-dark hover:text-primary"
                                >
                                    Polityka prywatności
                                </Link>
                                <a href="#" className="text-xs text-text-secondary-dark hover:text-primary">
                                    Regulamin
                                </a>
                            </div>*/}
                        </div>
                    </div>
                </Container>
            </div>
        </footer>
    );
};

export default Footer;